...
static int parse_fp(const char *buf, u64 *val)
{
	...
	// zastosowanie wykładnika dziesiętnego
	if (exp > 0) {
		int i;
		for (i = 0; i < exp; i++) {
			if (v > (U64_MAX / 10)) return -EINVAL;
	...
	u64 mantissa = v - (1ULL << 36);
	int fin_exp = bin_exp + (int)BIAS;
...
static int format_fp(u64 val, char *buf, size_t size)
{
	...
	u64 mant_field = (val >> 28) & 0xFFFFFFFFFULL;	// 36 bitów
	u64 mant_full = (1ULL << 36) | mant_field;		// 1.xxx (37 bitów)

	int real_exp = (int)exp_field - (int)BIAS;
	int bin_exp_adj = real_exp - 36;			// wykładnik potęgi 2

	u64 dec_val = mant_full;
	int dec_exp = 0;

	// przeliczamy 2^b na potęgi 10
	while (bin_exp_adj > 0) {
	...
	// zamiana na string i normalizacja do jednej cyfry przed kropką
	char tmp[32];
...
static ssize_t status_read(struct file *f, char __user *ubuf,
			   size_t cnt, loff_t *off)
{
	if (*off) return 0;
...
static ssize_t result_read(struct file *f, char __user *ubuf,
			   size_t cnt, loff_t *off)
{
	if (*off) return 0;
